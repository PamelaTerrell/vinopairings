...
export default function CelestialSipsPage() {
  ...
  // Season-aware (UTC month keeps it static-friendly)
  ...
  const seasonCopy = {
    Winter: {
      lead: 'As nights draw in and the air stills, we crave warmth, resonance, and the kind of depth that lingers.',
      feature: 'Featured pairing: Syrah with braised short ribs and rosemary — dark fruit, pepper, and candlelight.',
      notes: ['Savory reds', 'Slow braises', 'Fireside conversation'],
    },
    Spring: {
      lead: 'Light returns and the table brightens. Freshness, lift, and herbs guide the glass and the gathering.',
      feature: 'Featured pairing: Sauvignon Blanc with herbed salmon — citrus, garden greens, easy laughter.',
      notes: ['Crisp whites', 'Young greens', 'Lemon & dill'],
    },
    Summer: {
      lead: 'Long evenings, open windows, and bowls of ripe produce. Texture softens; the wine turns playful.',
      feature: 'Featured pairing: Dry Rosé with grilled vegetables — sun–warmed fruit and a cool breeze.',
      notes: ['Rosé & spritz', 'Chilled reds', 'Stone fruit & smoke'],
    },
    Autumn: {
      lead: 'Harvest hues, a quieter pace, and flavors that lean earthy and wise. Depth without weight.',
      feature: 'Featured pairing: Pinot Noir with roast poultry & mushrooms — silk, forest, and a gentle glow.',
      notes: ['Mushroom & thyme', 'Silky reds', 'Roast suppers'],
    },
  }[season];

  // [slug, Name, Wine, card note, essay]
  const signs = [
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: brand.cream, color: brand.cocoa }}>
      <main id="top" className="mx-auto w-full max-w-6xl px-6 py-12 md:py-16">
        ...
        >
          <div aria-hidden="true" className="absolute inset-x-0 top-0 h-64"
        ...
        {/* READ MORE — TOC + anchored essays */}
        <section className="mt-16 rounded-2xl border p-8 shadow-sm relative" style={{ backgroundColor: brand.parchment, borderColor: brand.line }}>
          <h2 className="text-3xl font-serif font-bold tracking-wide text-center">Read More</h2>

          {/* Mini Table of Contents */}
          <nav aria-label="Zodiac table of contents" className="mt-6 rounded-xl border bg-white p-4 shadow-sm" style={{ borderColor: brand.line }}>
            <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {signs.map(([slug, name]) => (
                <li key={slug}>
                  <a href={`#${slug}`} data-sign={name} className="block rounded-md px-3 py-2 text-sm hover:bg-[#fdfaf3] transition-colors" style={{ color: brand.cocoa }}>
                    {name}
                  </a>
                </li>
              ))}
          ...
          <div className="mt-8 space-y-10">
            {signs.map(([slug, name, wine, , essay]) => (
              <article key={slug} id={slug} className="rounded-xl border bg-white p-6 shadow-sm scroll-mt-24" style={{ borderColor: brand.line }}>
          ...
          {/* 🌟 Floating "Back to Gallery" button (pure CSS visibility) */}
          <a
            href="#top"
            id="backToGallery"
            className="fixed right-5 bottom-5 translate-y-6 opacity-0 transition-all duration-500 ease-out rounded-full shadow-md px-5 py-2 text-sm font-semibold"
            style={{ backgroundColor: brand.gold, color: '#fff', border: `1px solid ${brand.line}`, zIndex: 50 }}
          >
            ⤴ Back to Gallery
          </a>

          {/* Plain CSS (allowed in Server Component) */}
          <style>{`
            /* Fade-in for floating button once body is marked as being in an essay */
            body[data-celestial-in-essay="1"] #backToGallery {
              opacity: 1;
        ...
        <section className="mt-16 rounded-2xl border p-8 text-center shadow-sm" style={{ borderColor: brand.line, background: '#fff' }}>
          ...
          <p className="mx-auto mt-3 max-w-2xl text-lg">
            ...
            <a href="https://cosmiczodiacastrology.com" target="_blank" rel="noopener noreferrer" className="underline decoration-dotted underline-offset-4 hover:opacity-90" style={{ color: brand.gold }}>
      ...
      {/* GA4: page view + sign-click tracking + essay state toggle */}
      <script
          ...
          __html: `
            (function(){
              try {
                ...
                // Track clicks on gallery/TOC sign links
                ...
                // Toggle essay state (controls floating button visibility)
                var slugs = ${JSON.stringify(signs.map(s => s[0]))};
                function updateEssayState(){
                  var h = (location.hash || '').replace('#','');
                  if (slugs.indexOf(h) !== -1) {
                    document.body.setAttribute('data-celestial-in-essay','1');
                  } else {
                    document.body.removeAttribute('data-celestial-in-essay');
                  }
                }
                window.addEventListener('hashchange', updateEssayState, { passive: true });
                document.addEventListener('DOMContentLoaded', updateEssayState, { passive: true });
                updateEssayState();
              } catch(e){}
            })();
          `,
