...
// Cards that route to top-level pairing pages (slug) or nested region pages (href)
const regions = [
  ...
  // Emerging Asia page lives under /regions
  { href: "/regions/asia", name: "Asia (Emerging Regions)", blurb: "China • India • Thailand • Japan", emoji: "🌏" },
];
...
export default function RegionsPage() {
  return (
    <section className="py-6">
      {/* HERO */}
      <div className="relative overflow-hidden rounded-3xl border border-[#d8cfc4] bg-white shadow-sm">
        {/* soft gradient wash */}
        ...
        {regions.map((r) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="group rounded-2xl border border-[#d8cfc4] bg-white p-5 shadow-sm transition hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#d8cfc4]"
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-[#6e2a2a] group-hover:underline">
                    {r.name}
                  </h2>
                  <p className="mt-1 text-[#4b3f2f]/80">{r.blurb}</p>
                </div>
              </div>
            </Link>
          );
