...
export default function HomeClient() {
  ...
  const computeResult = (q, mode) => {
    ...
    if (mode === "dish") {
      ...
      const bestDish = dishes
        .filter((dish) => hasAnyTokenSignal(q, dish))
        .map((dish) => ({
          dish,
          score: scoreCandidate(q, dish),
        }))
      ...
      return {
        found: true,
        text: `🍷 A beautiful wine pairing for "${bestDish.dish}" is **${pairings[bestDish.dish]}**.`,
      };
    }
    ...
    if (mode === "wine") {
      ...
      const bestWine = wines
        .filter((wine) => hasAnyTokenSignal(q, wine))
        .map((wine) => ({
          wine,
          score: scoreCandidate(q, wine),
        }))
        .sort((a, b) => b.score - a.score)[0];
      ...
      return {
        found: true,
        text: `🍽️ Delicious dishes to enjoy with "${bestWine.wine}" include: ${dishes}.`,
      };
    }
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-[190px_1fr]">
            ...
            <div className="relative flex flex-col gap-2">
              ...
              <input
                ...
                placeholder={
                  type === "dish"
                    ? "e.g., pasta, salmon, steak, Alfredo"
